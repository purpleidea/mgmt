...
.PHONY: mkosi mkosi_fedora-30 mkosi_fedora-29 mkosi_centos-8 mkosi_centos-7 mkosi_debian-10 mkosi_ubuntu-bionic mkosi_archlinux
.PHONY: release releases_path release_fedora-30 release_fedora-29 release_centos-8 release_centos-7 release_debian-10 release_ubuntu-bionic release_archlinux
.PHONY: funcgen
...
FILE_CENTOS-8 = mgmt-$(TOKEN_CENTOS-8)-$(VERSION)-1.x86_64.rpm
...
PKG_CENTOS-8 = releases/$(VERSION)/$(TOKEN_CENTOS-8)/$(FILE_CENTOS-8)
...
#	mkosi
#
mkosi: mkosi_fedora-30 mkosi_fedora-29 mkosi_centos-8 mkosi_centos-7 mkosi_debian-10 mkosi_ubuntu-bionic mkosi_archlinux ## builds distro packages via mkosi
...
mkosi_centos-8: releases/$(VERSION)/.mkdir
	@title='$@' ; echo "Generating: $${title#'mkosi_'} via mkosi..."
	@title='$@' ; distro=$${title#'mkosi_'} ; ./misc/mkosi/make.sh $${distro} `realpath "releases/$(VERSION)/"`
...
releases/$(VERSION)/mgmt-release.url: $(PKG_FEDORA-30) $(PKG_FEDORA-29) $(PKG_CENTOS-8) $(PKG_CENTOS-7) $(PKG_DEBIAN-10) $(PKG_UBUNTU-BIONIC) $(PKG_ARCHLINUX) $(SHA256SUMS_ASC)
...
releases/$(VERSION)/.mkdir:
	mkdir -p releases/$(VERSION)/{$(TOKEN_FEDORA-30),$(TOKEN_FEDORA-29),$(TOKEN_CENTOS-8),$(TOKEN_CENTOS-7),$(TOKEN_DEBIAN-10),$(TOKEN_UBUNTU-BIONIC),$(TOKEN_ARCHLINUX)}/ && touch releases/$(VERSION)/.mkdir
...
releases/$(VERSION)/$(TOKEN_CENTOS-8)/changelog: $(PROGRAM) releases/$(VERSION)/.mkdir
	@title='$(@D)' ; distro=$${title#'releases/$(VERSION)/'} ; echo "Generating: $${distro} changelog..."
	@title='$(@D)' ; distro=$${title#'releases/$(VERSION)/'} ; ./misc/make-rpm-changelog.sh "$${distro}" $(VERSION)

$(PKG_CENTOS-8): releases/$(VERSION)/$(TOKEN_CENTOS-8)/changelog
	@title='$(@D)' ; distro=$${title#'releases/$(VERSION)/'} ; echo "Building: $${distro} package..."
	@title='$(@D)' ; distro=$${title#'releases/$(VERSION)/'} ; ./misc/fpm-pack.sh $${distro} $(VERSION) "$(FILE_CENTOS-8)" libvirt-devel augeas-devel
...
$(SHA256SUMS): $(PKG_FEDORA-30) $(PKG_FEDORA-29) $(PKG_CENTOS-8) $(PKG_CENTOS-7) $(PKG_DEBIAN-10) $(PKG_UBUNTU-BIONIC) $(PKG_ARCHLINUX)
	@# remove the directory separator in the SHA256SUMS file
	@echo "Generating: sha256 sum..."
	sha256sum $(PKG_FEDORA-30) $(PKG_FEDORA-29) $(PKG_CENTOS-8) $(PKG_CENTOS-7) $(PKG_DEBIAN-10) $(PKG_UBUNTU-BIONIC) $(PKG_ARCHLINUX) | awk -F '/| ' '{print $$1"  "$$6}' > $(SHA256SUMS)
