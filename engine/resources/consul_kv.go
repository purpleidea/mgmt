...
// server address and the key name: http://127.0.0.1/my-key.
...
func (obj *ConsulKVRes) Init(init *engine.Init) error {
	...
	s, a, k := parseConsulName(obj.Name())
	if obj.Key == "" {
		obj.Key = k
	}
	if obj.Address == "" {
...
// parseConsulKVName parses the Name() of a resource and extracts the scheme,
// address, and key name of a consul key.
func parseConsulName(name string) (string, string, string) {
