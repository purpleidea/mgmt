...
type DockerContainerRes struct {
	...
	// Networks
	Networks []string `lang:"networks" yaml:"networks"`

	NetworkMode string `lang:"network_mode" yaml:"network_mode"`

	// Entrypoint
	Entrypoint []string `lang:"entrypoint" yaml:"entrypoint"`
	...
	Devices []string `lang:"devices" yaml:"devices"`
	// Domainname is the Domain name of the container
	Domainname *string `lang:"domainname" yaml:"domainname"`
	...
	Ports map[string]map[int64]string `lang:"ports" yaml:"ports"`
	// portSet
	portSet nat.PortSet
	// portMap
	portMap nat.PortMap
	// Restart is the policy used to determine how to restart the container
	Restart *string `lang:"restart" yaml:"restart"`
	...
	WorkingDir *string `lang:"workdir" yaml:"workdir"`
...
func (obj *DockerContainerRes) CheckApply(ctx context.Context, apply bool) (bool, error) {
	...
	if obj.State == ContainerAbsent { // container exists and should be absent
		...
		return false, obj.containerRemove(ctx, ctr.ID, container.RemoveOptions{Force: true})
	}
	...
	if len(containerList) == 0 { // no container was found
		...
		if err != nil {
			return false, errwrap.Wrapf(err, "error inspecting container: %s", c.ID)
		}
	}
	...
	return false, obj.containerStart(ctx, ctr.ID, container.StartOptions{})
}
