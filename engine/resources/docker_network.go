...
type DockerNetworkRes struct {
	...
	EnableIPv6 bool `lang:"enableipv6" yaml:"enableipv6"`
	// Labels are key=value pairs to label the network with
	Labels map[string]string `lang:"labels" yaml:"labels"`
	...
	IPAMDriver string `lang:"ipamdriver" yaml:"ipamdriver"`
	// IPAMOptions are key=value pairs to pass to the IPAM driver
	IPAMOptions map[string]string `lang:"ipamoptions" yaml:"ipamoptions"`
...
func (obj *DockerNetworkRes) Cleanup() error {
	return nil
}

// Watch is the primary listener for this resource and it outputs events.
func (obj *DockerNetworkRes) Watch(ctx context.Context) error {
	var docker *client.Client
	var err error

	for {
		docker, err = client.NewClientWithOpts(client.WithVersion(obj.APIVersion))
		if err == nil {
		...
		if client.IsErrConnectionFailed(err) && !obj.sflag {
...
func (obj *DockerNetworkRes) CheckApply(ctx context.Context, apply bool) (checkOK bool, err error) {
	...
	var id string
	nets, err := obj.client.NetworkList(ctx, network.ListOptions{
	...
	if obj.State == "exists" && id != "" {
		inspect, err := obj.client.NetworkInspect(ctx, id, network.InspectOptions{})
		if err != nil {
...
func (obj *DockerNetworkRes) cmpNetwork(ctx context.Context, inspect network.Inspect) error {
...
type DockerNetworkAddressRes struct {
	...
	Subnet     string            `lang:"subnet" yaml:"subnet"`
	IPRange    string            `lang:"iprange" yaml:"iprange"`
	Gateway    string            `lang:"gateway" yaml:"gateway"`
	AuxAddress map[string]string `lang:"auxaddress" yaml:"auxaddress"`
}
...
func (obj *DockerNetworkAddressRes) Cleanup() error {
	return nil
}

// Watch has no function for DockerContainerMount resources
func (obj *DockerNetworkAddressRes) Watch(ctx context.Context) error {
	if err := obj.init.Event(ctx); err != nil {
		return err
	}
	<-ctx.Done()
...
func (obj *DockerNetworkAddressRes) CheckApply(ctx context.Context, apply bool) (bool, error) {
