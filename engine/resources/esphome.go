...
var esphomeNamedColours = map[string]uint64{
...
// EsphomeLightRes manages an RGB esphome light. Colour accepts a small stable
// vocabulary of names or an exact #RRGGBB value; parseEsphomeColour is kept
...
type EsphomeLightRes struct {
	...
	// Colour is a supported name or an exact #RRGGBB value. The entity must
	// advertise RGB colour-mode support when the desired state is on.
	Colour string `lang:"colour" yaml:"colour"`
	...
	// and show the plain Colour instead.
...
// parseEsphomeColour converts a stable colour name or #RRGGBB value to native
...
func parseEsphomeColour(value string) (float64, float64, float64, error) {
	normalized := strings.ToLower(strings.TrimSpace(value))
	rgb, ok := esphomeNamedColours[normalized]
	if !ok {
		if len(normalized) != 7 || normalized[0] != '#' {
			return 0, 0, 0, fmt.Errorf("colour must be a supported name or #RRGGBB")
		}
		var err error
		rgb, err = strconv.ParseUint(normalized[1:], 16, 24)
		if err != nil {
			return 0, 0, 0, fmt.Errorf("invalid colour %q: %v", value, err)
		}
	}
...
// omit optional colour capabilities so any managed light can be turned off.
func (obj *EsphomeLightRes) command(on bool) (esphomeUtil.LightCommand, error) {
	red, green, blue, err := parseEsphomeColour(obj.Colour)
	if err != nil {
...
func (obj *EsphomeLightRes) Default() engine.Res {
	return &EsphomeLightRes{Brightness: 1, Colour: "white"}
}
...
func (obj *EsphomeLightRes) Validate() error {
	...
	if _, _, _, err := parseEsphomeColour(obj.Colour); err != nil {
		return err
	}
	if obj.getId() == "" {
		return fmt.Errorf("empty id")
...
func (obj *EsphomeLightRes) CheckApply(ctx context.Context, apply bool) (bool, error) {
	...
	obj.init.Logf("turning light %s at brightness %v with colour %s and effect %s", obj.State, obj.Brightness, obj.Colour, obj.Effect)
...
func (obj *EsphomeLightRes) Cmp(r engine.Res) error {
	...
	if obj.Colour != res.Colour {
		return fmt.Errorf("the Colour differs")
	}
