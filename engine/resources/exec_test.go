...
func TestExecGetCancelSignal(t *testing.T) {
	testCases := []struct {
		name   string
		signal string
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			obj := &ExecRes{
				Cmd:          "true",
				CancelSignal: tc.signal,
			}
			signal, err := obj.getCancelSignal()
			if tc.fail {
				if err == nil {
					t.Fatalf("func getCancelSignal: expected error, got signal: %v", signal)
				}
				if tc.hint != "" && !strings.Contains(err.Error(), tc.hint) {
					t.Errorf("func getCancelSignal: got error: %v, expected hint: %s", err, tc.hint)
				}
			...
			if err != nil {
				t.Fatalf("func getCancelSignal: %v", err)
			}
			if signal != tc.expect {
...
// TestExecCancelSignal tests that cancelling the context asks the command to
// stop with a signal it can catch, and that a command which catches it and then
// exits with a zero status is still an error, because it never did the work
// that we asked it to do.
func TestExecCancelSignal(t *testing.T) {
	testCases := []struct {
		name   string
		signal string // the CancelSignal param
