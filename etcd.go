...
func (obj *EmbdEtcd) Startup() error {
	...
	obj.Connect(false) // don't exit from this Startup function until connected!
	return nil
}
...
func (obj *EmbdEtcd) CtxError(ctx context.Context, err error) (context.Context, error) {
	...
	if obj.exiting {
		return ctx, fmt.Errorf("Etcd: CtxError: Exit in progress!")
	}
	...
	if retriesErr, ok := err.(*CtxRetriesErr); ok { // custom retry error
		...
		if retriesErr.Retries == 0 {
			return ctx, fmt.Errorf("Etcd: CtxError: CtxRetriesErr: No more retries!")
		}
		return ctx, nil
	}
	...
	return ctx, fmt.Errorf("Etcd: CtxError: Unknown error!")
}
	...
	for {
		...
		case re := <-obj.wevents:
			...
			for {
				...
				// FIXME: look at return value of CtxError()
				ctx, _ = obj.CtxError(ctx, err)
			}
			if TRACE {
...
func (obj *EmbdEtcd) StartServer(newCluster bool, peerURLsMap etcdtypes.URLsMap) error {
	...
	obj.etcdConn = NewEtcdConn( // XXX: get this upstream!
		...
		obj.dataDir, nil, // XXX: is this the same datadir or not?
	)
...
func (obj *EmbdEtcd) DestroyServer() error {
	...
	if obj.server != nil {
		obj.server.Stop() // this blocks until server has stopped
	}
