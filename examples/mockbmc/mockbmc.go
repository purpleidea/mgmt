...
func (obj *MockBMC) Run() error {
	...
	if obj.Driver == "redfish" || obj.Driver == "gofish" {
		...
		http.HandleFunc("/redfish/v1/Systems/1", obj.endpointFunc("systems_1.json", http.MethodGet, 200, nil))
...
// wget --no-check-certificate --post-data 'user=foo&password=bar' https://localhost:8800/redfish/v1/Systems/1/Actions/ComputerSystem.Reset -O -
func main() {
	fmt.Printf("main: %+v\n", Main())
}
