...
func (obj *RangeFunc) FuncInfer(partialType *types.Type, partialValues []types.Value) (*types.Type, []*interfaces.UnificationInvariant, error) {
	...
	l := len(partialValues)
	if l < 1 || l > 3 {
		return nil, nil, fmt.Errorf("function must have between 1 to 3 args")
	}

	var typ *types.Type
	if l == 1 {
		// we only have stop argument
		typ = types.NewType("func(int) []int")
	}

	if l == 2 {
		// we have start and stop argument
...
func (obj *RangeFunc) Call(ctx context.Context, args []types.Value) (types.Value, error) {
	...
	if len(args) == 1 { // we only have stop, assume start is 0 and step is 1
		result, err = obj.run(ctx, 0, args[0].Int(), 1)
		if err != nil {
			return nil, err
		}
	}
	if len(args) == 2 { // we have start and stop, assume step is 1
		result, err = obj.run(ctx, args[0].Int(), args[1].Int(), 1)
		if err != nil {
			return nil, err
		}
	}
	if len(args) == 3 { // we have all the args
		result, err = obj.run(ctx, args[0].Int(), args[1].Int(), args[2].Int())
		if err != nil {
			return nil, err
		}
	}
	// TODO: should call the for function here and return the result
	return &types.ListValue{V: result}, nil
}

// Run is the private helper function that calculates the range according to the
// inputs provided.
func (obj *RangeFunc) run(ctx context.Context, start, stop, step int64) ([]types.Value, error) {
	if step == 0 {
	...
	if step > 0 {
		...
		// calculating the lenght needed for initializing the list
	...
	result := make([]types.Value, 0, length)
