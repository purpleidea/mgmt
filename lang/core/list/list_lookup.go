...
func init() {
	simple.ModuleRegister(ModuleName, ListLookupFuncName, &simple.Scaffold{
		I: &simple.Info{
			Pure: true,
			Memo: true,
			Fast: true,
...
// ListLookup returns the value at the input index in the list. If the index is
// not present, then this errors with a catchable (sentinel) error, so that the
// except operator can catch it and provide a fallback value, eg:
//
//	list.lookup($list, 42) <|> "default"
//
// Passing a negative index will error similarly.
