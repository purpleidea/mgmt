...
// virtualizationVendorSlice is a slice of strings that might be found on DMI
...
// Check if any of the keys of virtualizationVendorSlice are present in any of
...
func dmiFileCheck(ctx context.Context) (bool, error) {
	for _, dmiFile := range dmiFilesSlice {
		...
		if err != nil && !os.IsNotExist(err) {
			return false, fmt.Errorf("error with the DMI file: %s", dmiFile)
		} else if err != nil {
			continue
		}
