...
func (obj *MapFunc) Polymorphisms(partialType *types.Type, partialValues []types.Value) ([]*types.Type, error) {
	...
	tF := types.NewType(fmt.Sprintf("func(%s) %s", tI.String(), tO.String()))
...
func (obj *MapFunc) Build(typ *types.Type) error {
	...
	if err := tArg.Cmp(tInputs.Val); err != nil {
		return errwrap.Wrapf(err, "the functions arg type must match the input arg type")
	}
...
func (obj *MapFunc) Info() *interfaces.Info {
	...
	tI := types.NewType(fmt.Sprintf("[]%s", tIi.String()))

	tOi := types.TypeVariant
	if obj.RType != nil {
		tOi = obj.RType
	}
	tO := types.NewType(fmt.Sprintf("[]%s", tOi.String()))
