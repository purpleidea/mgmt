...
func init() {
	funcs.ModuleRegister(ModuleName, "exec", func() interfaces.Func { return &ExecFunc{} })
}

// ExecFunc runs a string as a shell command, then produces each line from
...
func (obj *ExecFunc) ArgGen(index int) (string, error) {
...
func (obj *ExecFunc) Validate() error {
	return nil
}

// Info returns some static info about itself.
func (obj *ExecFunc) Info() *interfaces.Info {
...
func (obj *ExecFunc) Init(init *interfaces.Init) error {
	obj.init = init
...
func (obj *ExecFunc) Stream() error {
		...
		case input, more := <-obj.init.Input:
			...
			go func() {
				...
				for stderrScanner.Scan() {
					obj.init.Logf("ExecFunc: \"%v\": stderr: %v\n", shellCommand, stderrScanner.Text())
				}
			}()
