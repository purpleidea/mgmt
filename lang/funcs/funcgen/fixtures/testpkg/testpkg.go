...
// Lgamma returns the natural logarithm and sign of Gamma(x).
// Two non-error results.
//
// REJECT (by generator policy)
...
// Nextafter32 uses float32 in params/results
// float32 type is not in the allowlist.
//
// REJECT
...
// SuperByte returns []byte
// The generator keeps []byte so the template applies string(...).
//
// ALLOW
...
// Join is a variadic example (...string) represented as []string + Variadic=true.
//
// ALLOW
