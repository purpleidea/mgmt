...
// parsefuncs discovers exported top-level functions for obj.Name using go/packages and go/types.
...
//   - Calls mapType with the given policy (param vs result).
//   - Synthesizes an arg with a name (from the source, or argN fallback).
//
// Returns:
//   - The full []arg slice if all element types are supported.
//   - ok=false if any element type is unsupported, in which case we skip
//     generating this function entirely.
...
//  1. Exactly one result: of a supported simple type (mapType with resultPolicy).
//  2. Exactly two results: (T, error) where T is a supported simple type and the second result is the error type.
//
// Anything else (0 results, >2 results, multiple non-error results, etc.) is rejected.
//
// Returns:
//   - []arg representing the non-error return(s).
//   - a bool 'errorful' which is true if the function is (T, error).
//   - ok=false if the results don’t match the allowed shapes.
...
//
// It returns:
//   - the generator type string we use in templates ("bool", "string", "int64", "[]byte", "[]string", ...)
//   - a boolean ok flag (false means: type not supported, skip this function).
//
// The behavior depends on `policy`:
//   - paramPolicy: parameters are more permissive. Both []byte and []string are accepted
//     (the latter is used to model variadic ...string).
//   - resultPolicy: results are stricter. []byte is allowed (so the template can wrap it with string(...)),
//     but []string is rejected (no runtime support).
	...
	case *types.Slice:
		...
		if b, ok := bt.Elem().(*types.Basic); ok {
			...
			case types.Byte: // alias for uint8
				// Params: accept []byte; Results: keep as []byte (template adds string(...)).
				return "[]byte", true
			case types.String:
...
// loadPackage resolves and loads a single package import path with types and syntax.
...
// collectFuncDocs scans a package’s AST and returns doc comments for top-level functions.
// Ignoring methods with receivers.
