...
// ForFunc receives a list from upstream. XXX expand docs
type ForFunc struct {
	...
	lastForList         types.Value // remember the last value
...
func (obj *ForFunc) Info() *interfaces.Info {
	...
	if obj.IndexType != nil && obj.ValueType != nil { // don't panic if called speculatively
		...
		// XXX TEMP FLOAT TO PROVE WE'RE DROPPING THE OUTPUT SINCE WE DON'T NEED IT
...
func (obj *ForFunc) Stream(ctx context.Context) error {
	...
	subgraphInput := &ChannelBasedSourceFunc{
		...
		Source: obj,
		Chan:   inputChan,
		Type:   types.NewType(fmt.Sprintf("[]%s", obj.ValueType)),
	}
	...
	for {
		...
		case input, ok := <-obj.init.Input:
			...
			fmt.Printf("XXX GOT A LIST FROM INPUT: %+v\n", input) // XXX DEBUG
			...
			// If we have a new list or the length of the input list
			// has changed, then we need to replace the subgraph
			// with a new one that uses the new list the correct
			// number of times.
			...
			if forList.Cmp(obj.lastForList) != nil || n != obj.lastInputListLength {
...
func (obj *ForFunc) replaceSubGraph(subgraphInput interfaces.Func) error {
	...
	for i := 0; i < obj.lastInputListLength; i++ {
		i := i
		argName := "inputList"
		// XXX: TO WORKAROUND THE TXN BUG DO:
		argName = fmt.Sprintf("%s%d", argName, i) // XXX !!!
