	...
	for {
		...
		case 56:
			{ // this matches a (#) pound char followed by any
				...
				// We don't return a COMMENT token (which would need
				// grammar support everywhere) but we instead collect
				// the comments in a side-channel list so that tools
				// like the code formatter can re-attach them by pos.
				lp := yylex.cast()
				lp.comments = append(lp.comments, &interfaces.Comment{
					Value: lval.str,
					Row:   yylex.Line(),
					Col:   yylex.Column(),
				})
