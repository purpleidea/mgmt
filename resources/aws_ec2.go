...
func (obj *AwsEc2Res) longpollWatch() error {
	...
	go func() {
		...
		for {
			...
			state, err := longpollWaiter(ctx, instance, obj.client)
			if err != nil {
				select {
				case obj.awsChan <- &chanStruct{
	...
	for {
		...
		case msg, ok := <-obj.awsChan:
			...
			switch msg.state {
			// send events for non-transitional states only.
...
// longpollWaiter waits for an instance to change state, and returns the state.
func longpollWaiter(ctx context.Context, instance *ec2.Instance, c *ec2.EC2) (string, error) {
	var err error
	var name string
	...
	// instance.InstanceId might be nil in case of terminated instances, which
	// the api will treat as if it isn't included.
	...
	// call on the appropriate waiter based on current state.
