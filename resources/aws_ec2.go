...
const (
	...
	// DefaultClientPort is the default client port for etcd connections.
	DefaultClientPort = 2379
	// DefaultServerPort is the default server port for etcd connections.
...
// bootstrapScript is a bash script template for installing mgmt and connecting
// back to the seed specified in the definition.
const bootstrapScript string = `#!/bin/bash
if [ -n "$(command -v apt)" ]
then
	apt update
	apt install -y libaugeas0 libvirt0
fi

if [ -n "$(command -v yum)" ]
then
	yum install -y augeas-libs libvirt-libs
fi
...
${tmpdir}/mgmt run --yaml graph.yaml --no-pgp --tmp-prefix \
--seeds {{.Seed}} \
...
func (*AwsEc2Res) bootstrapUserData(binary, graph, seed string) (string, error) {
	// create a template for the user data
	buf := new(bytes.Buffer)
	tmpl, err := template.New("Bootstrap Template").Parse(bootstrapScript)
	if err != nil {
	...
	bootstrapData := bootstrapTemplateData{
		...
		RemoteGraph: graph,
		Seed:        seed,
		CPort:       DefaultClientPort,
		SPort:       DefaultServerPort,
	}
