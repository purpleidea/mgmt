...
const (
	...
	PoweroffTimeout   = 90 // same as default systemd timeout
)
...
func (obj *NspawnRes) CheckApply(apply bool) (checkok bool, err error) {
	...
	if obj.State == stopped {
		...
		log.Printf("%v[%v]: Terminating machine", obj.Kind(), obj.GetName())
		retries := 5
		for retries > 0 {
			if err := conn.TerminateMachine(obj.GetName()); err != nil {
				...
				// fully started. We should wait a moment and
				// try again.
				if err.Error() != "Resource deadlock avoided" {
					errwrap.Wrap(err, "Failed to terminate machine")
					return false, err
				}
			}
