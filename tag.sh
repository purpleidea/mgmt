...
if [[ $# -gt 0 ]]; then
	err "$0 does not take arguments." >&2
	exit 1
fi

# Never tag a dirty git tree.
if is_git_dirty; then
	err "git tree is dirty. Commit or stash changes before tagging." >&2
	exit 1
fi

# Be idempotent.
if [[ "${h}" == "${v}" ]]; then
	info "HEAD \"${h}\" is equivalent to the current (old) version tag \"${v}\"; nothing to do."
	exit 0
fi

# Give the user a chance to abort.
warn "About to tag \"${h}\" as \"${t}\" and push."
echo "Press ^C within 3s to abort."
sleep 3s

# Tag and push.
echo "release: tag $t" | git tag --file=- --sign $t
info "Version $t is now tagged."
info "Pushing $t to origin."
git push origin $t
