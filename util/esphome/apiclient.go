...
func (obj *apiClientDriver) entities() ([]*EntityInfo, error) {
	...
	for _, e := range registry.Lights() {
		modes := make([]string, 0, len(e.SupportedColourModes))
		for _, mode := range e.SupportedColourModes {
			modes = append(modes, apiclient.ColourModeName(mode))
		}
		result = append(result, &EntityInfo{
			Key: e.Key, ObjectID: e.ObjectID, Name: e.Name, Domain: DomainLight,
			LightSupportedColourModes: modes,
		})
	}
	return result, nil
}
...
// close. The library records the first redacted, typed cause, so wrapping it
...
func (obj *apiClientDriver) setLight(key uint32, command LightCommand) error {
	return obj.client.SetLight(key, apiclient.LightCommandOpts{
		...
		HasColourMode: command.HasRGB,
		ColourMode:    apiclient.ColourModeRGB,
