	...
	d := &fakeDriver{
		entityList: []*EntityInfo{
			...
			},
			{
				...
				LightSupportedColourModes: []string{LightColourModeRGB},
			},
		},
		initial: []*EntityState{
...
func TestSessionCapabilitiesRejectUnsupportedCommands(t *testing.T) {
	factory := &fakeFactory{prepare: func(d *fakeDriver) {
		for _, entity := range d.entityList {
			...
			case DomainLight:
				entity.LightSupportedColourModes = []string{"COLOUR_MODE_BRIGHTNESS"}
			}
		}
	}}
	session := testSession(t, factory)
...
func TestSessionQueueOverflowCauseSurvivesWrapping(t *testing.T) {
	// A slow mgmt consumer ends the connection with ErrEventQueueFull. That
	// is actionable configuration feedback, so the sentinel must survive
	// the session's wrapping.
	...
	if err := session.LastError(); !errors.Is(err, apiclient.ErrEventQueueFull) {
		t.Fatalf("queue overflow sentinel did not survive wrapping: %v", err)
	}
}
