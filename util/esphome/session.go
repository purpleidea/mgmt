	...
	for key, identifiers := range obj.index {
		for _, id := range identifiers {
			if id == identifier {
				...
				copy := *entity
				copy.LightSupportedColourModes = append([]string(nil), entity.LightSupportedColourModes...)
				return &copy
			}
		}
	}
...
func (obj *Session) ValidateLightCommand(identifier string, command LightCommand) error {
	...
	for _, mode := range entity.LightSupportedColourModes {
		if mode == LightColourModeRGB {
			return nil
		}
	}
	return fmt.Errorf("light `%s` does not support RGB colour mode (supports: %v)", identifier, entity.LightSupportedColourModes)
}
...
func (obj *Session) markConnected(entities []*EntityInfo) {
	...
	for _, e := range entities {
		copy := *e
		copy.LightSupportedColourModes = append([]string(nil), e.LightSupportedColourModes...)
...
// asyncCloseReason returns the redacted reason a driver connection died on its
